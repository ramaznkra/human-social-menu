...
export function initOrderStatusPoll(config) {
    ...
    const stepLabels = ['Beklemede', 'Hazırlanıyor', 'Masaya Doğru', 'Afiyet Olsun'];
    ...
    function updateProgressUI(step, label) {
        ...
        const percent = safeStep > 0 ? (safeStep / 4) * 100 : 0;
    ...
    function applyStatus(data) {
        ...
        if (elHint && finalStatuses.includes(data.status)) {
            elHint.textContent = 'Siparişiniz tamamlandı. Afiyet olsun!';
        }
